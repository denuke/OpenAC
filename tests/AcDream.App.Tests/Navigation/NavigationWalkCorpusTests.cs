...
public sealed class NavigationWalkCorpusTests
{
    ...
    [Fact]
    public void TheCorpusIsWalkedAsRecorded()
    {
        ...
        foreach (WalkCase walk in Cases)
        {
            ...
            {
                world = null;
                world = PublishedLandblock.Load(datDirectory, walk.Landblocks, []);
    ...
    private static WalkResult Walk(WalkCase walk, PublishedLandblock world, bool cutCorners)
    {
        ...
        string text = string.Create(
            ...
            + $"{body.Seconds:0.0} s, {body.TravelStops} stops, {body.WalkingSeconds:0.0} s walking; {corners}{doorText}; "
    ...
    private static void CompareWithRecord(IReadOnlyList<string> lines, [CallerFilePath] string source = "")
    {
        string path = Path.Combine(Path.GetDirectoryName(source)!, RecordFile);
        bool recorded = File.Exists(path);
        bool update = System.Environment.GetEnvironmentVariable("ACDREAM_UPDATE_WALK_CORPUS") == "1";
        if (!recorded || update)
        {
